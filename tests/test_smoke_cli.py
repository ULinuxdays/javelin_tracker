...
def test_cli_smoke(tmp_path, monkeypatch):
    ...
    assert "[esha | Javelin]" in log_result.stdout

    discus_log = runner.invoke(
        app,
        [
            "log",
            "--date",
    ...
    assert discus_log.exit_code == 0, discus_log.stdout

    shot_log = runner.invoke(
        app,
        [
            "log",
            "--date",
    ...
    assert shot_log.exit_code == 0, shot_log.stdout
    ...
    summary_shot = runner.invoke(app, ["summary", "--athlete", "esha", "--event", "shot"])
    assert summary_shot.exit_code == 0, summary_shot.stdout
    assert "PER-EVENT" in summary_shot.stdout
    assert "shot" in summary_shot.stdout.lower()

    export_dir = tmp_path / "exports"
    export_result = runner.invoke(
        app,
        ["export", "--to", str(export_dir), "--athlete", "esha", "--event", "javelin"],
    )
    ...
    assert export_payload and len(export_payload) == 1
    assert export_payload[0]["event"] == "javelin"
    assert "app_version" in export_payload[0]
    assert export_payload[0]["generated_at"]

    metadata_payload = json.loads(metadata_files[0].read_text(encoding="utf-8"))
    assert metadata_payload["application"] == "throws-tracker"
    assert metadata_payload["rows"] >= 1
    assert metadata_payload["filters"]["athlete"] == "esha"
    assert metadata_payload["filters"]["event"] == "javelin"
    ...
    assert weekly_result.exit_code == 0, weekly_result.stdout
    pdf_paths = list(reports_dir.glob("*.pdf"))
    assert any("discus" in path.stem for path in pdf_paths), "Expected discus weekly report PDF"
